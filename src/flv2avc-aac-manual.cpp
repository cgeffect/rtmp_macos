...
int main() {
    ...
    while (true) {
        ...
        if (tag_header.type == AUDIODATA) {
            uint8_t audio_tag_body[tag_header.size];
            size_t body_length = sizeof(audio_tag_body);
            size_t n = fread(audio_tag_body, 1, body_length, ifh);
            if (n < 0) {
                printf("SCRIPTDATAOBJECT error\n");
                return 0;
            }
            struct flv_audio_tag_header_t audio_tag_header = {0};
            flv_audio_tag_header_read(&audio_tag_header, audio_tag_body, body_length);
            const char *data = reinterpret_cast<const char *>(audio_tag_body); // 显式类型转换
            ...
            size_t n = fread(video_tag_body, 1, body_length, ifh);
            if (n < 0) {
                printf("SCRIPTDATAOBJECT error\n");
                return 0;
            }
            ...
            int size = flv_video_tag_header_read(&video_tag_header, video_tag_body, body_length);
