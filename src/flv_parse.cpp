...
int main() {
    ...
    // FLV file header
    fread((char *)&flv_header, 1, sizeof(FLV_HEADER), ifh);
    ...
    // process each tag
    do {
        long current_pos = ftell(ifh);
        printf("Current file pointer position before reading previoustagsize: %ld\n", current_pos);
        ...
        // 方式2
        // 通常在 FLV 文件中以大端字节序存储
//        byte buffer[sizeof(uint)];
//        if (fread(buffer, 1, sizeof(uint), ifh) != sizeof(uint)) {
//            break;
//        }
//        // 转字节序
//        previoustagsize = reverse_bytes(buffer, sizeof(uint));
        printf("Read previoustagsize: %u\n", previoustagsize);
        ...
        printf("tagheader_datasize: %d\n", tagheader_datasize);
        printf("tagheader_timestamp: %d\n", tagheader_timestamp);
        // 打印标签信息
        fprintf(myout, "[%6s] %6d %6d |", "TAG", tagheader_datasize, tagheader_timestamp);
        ...
        case TAG_TYPE_VIDEO: {
            ...
            if (vfh == NULL && output_v != 0) {
                ...
                fwrite((char *)&flv_header, 1, sizeof(flv_header), vfh);
    ...
    fclose(ifh);
    if (afh != NULL) fclose(afh);
    if (vfh != NULL) fclose(vfh);
